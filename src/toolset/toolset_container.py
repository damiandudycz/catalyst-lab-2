...
import os, subprocess, tempfile, uuid, shutil, stat, threading
...
class ToolsetContainer:
    ...
    @classmethod
    def container_for_toolset(cls, toolset: Toolset) -> Self:
        """Makes sure only one instance for toolset is ever created. Use this instead of init."""
        with cls.containers_lock:
    ...
    def spawn(self, store_changes: bool = False, hot_fixes: List[HotFix] | None = None, additional_bindings: List[BindMount] | None = None):
        ...
        with self.access_lock:
            if self.spawned:
                raise RuntimeError(f"ToolsetContainer for {self.toolset} already spawned.")

            # Prepare /tmp directories and bind_options
            runtime_env = RuntimeEnv.current()

            resolved_toolset_root = str(Path(self.toolset.toolset_root()).resolve())
